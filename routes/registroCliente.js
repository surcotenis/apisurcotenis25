...
//
    ...
    if(getPago.length === 0){
        ...
        if (updatePago.affectedRows === 0) {
          ...
          console.log('Estado actualizado correctamente',input.codRegistro)
          res.json({ ok: true, message:`Estado actualizado correctamente. ${input.codRegistro}` });
        }
    }else{
        console.error(`el pago ${input.codRegistro} ya fue registrado`)
        res.json({ ok: false, message: `el pago ${input.codRegistro} ya fue registrado` });
    }
...
function calPriceTime(horaInicio, horaFin, precioBase) {
  ...
  if (
    ...
    const resultado = parseFloat(precioBase) + 35; // Sumar 30 al precio base
